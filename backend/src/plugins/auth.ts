  ...
  const redisStore = {
    async set(sessionId: string, session: Record<string, any>, callback: (err: Error | null) => void) {
      try {
        ...
        await fastify.redis.set(`auth:session:${sessionId}`, JSON.stringify(session), 'EX', maxAgeSeconds);
    ...
    async get(
      ...
      try {
        const sessionData = await fastify.redis.get(`auth:session:${sessionId}`);
    ...
    async destroy(sessionId: string, callback: (err: Error | null) => void) {
      try {
        await fastify.redis.unlink(`auth:session:${sessionId}`);
        callback && callback(null);
      } catch (error) {
        callback && callback(error as Error);
      }
